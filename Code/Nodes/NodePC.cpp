...
{
	if (have_phys_connection && nodeState == NodeState::Captured && packet->packetType == PacketType::Informative && packet->sInformation->for_spy_ref)
	{
...
void ANodePC::GeneratePacket(int chance)
{
	auto generate_PC_ID = [my_id = this->id]() -> int
	{
		...
		if (id == my_id)
		{
			if (id + 1 != ANodePC::id_counter) return id + 1;
			if (id - 1 != ANodePC::id_counter) return id - 1;
			return -2;
		}
		return id;
	...
	*/
	...
	if (chance >= 90 && chance < 95)
	{
		...
		if (nodes && !(*nodes).empty())
		{
			...
			switch (std::rand() % (3))
			{
			case 0: packet->sInformation->roots_from_id = 70 + std::rand() % (ANodePC::id_counter - 70); break; //Root PC
			case 1: packet->sInformation->roots_from_id = 50 + std::rand() % (ANodeDS::id_counter - 50); break; //Root DS
			case 2: packet->sInformation->roots_from_id = 10 + std::rand() % (ANodeTI::id_counter - 10); break; //Root TI
			}
			SendPacket(packet, nodes, (*nodes).begin());
		}
	}
